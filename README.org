...
The =:general= keyword in =use-package= declarations is provided by =general.el=.
Since this file uses =:general= before our main =general= configuration block,
we must ensure that =general.el= is loaded early enough to register its keyword
handlers.

#+begin_src emacs-lisp :noweb yes
<<config-navigation-general>>
#+end_src
...
** Navigation
*** General
This block is tangled earlier to ensure =general.el= has registered the
=:general= keyword before it is first used.

#+name: config-navigation-general
#+begin_src emacs-lisp :tangle no
  (use-package general
    :demand t
    :config
